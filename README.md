...
- **Линза преломляет пиксели, а не DOM.** Сцену ей даёт снимок видимой области вкладки
  (`chrome.tabs.captureVisibleTab` в `src/background.js`) — иначе преломлять нечего и
  деталь вырождается в плашку. В линзе же живёт плотность тела по `legibility`: без неё
  панель нечем отделить от текста страницы под ней. На время снимка виджет прячется,
  чтобы стекло не начало преломлять само себя.

  Отсюда `host_permissions: ["<all_urls>"]`: по конкретным сайтам `captureVisibleTab` не
  работает, ему нужен именно этот доступ. Снимок никуда не уходит — он живёт в текстуре
  на той же странице и не покидает вкладку.
...
src/background.js      — снимок вкладки для линзы, дефолтный формат
```
...
не отправляет**: сборка книги, снимок вкладки для стекла и сам файл не покидают машину.
Сетевых вызовов наружу нет вовсе. Скачивание — для личного чтения.

Про `<all_urls>` стоит сказать честно: формально это доступ «читать всё на всех сайтах»,
и просит его именно `captureVisibleTab` ради фона под панелью. Контент-скрипты при этом
запускаются только на сайтах из `matches`, снимок остаётся во вкладке, и никакого канала
наружу в коде нет — проверяется поиском по `fetch` и `XMLHttpRequest`. Не устраивает —
удалите `host_permissions` из `manifest.json`: расширение продолжит собирать книги, стекло
станет плоским.

// EPUB 3 with an NCX for backwards compatibility: Send to Kindle converts this one.
...
const escA = (s: unknown): string => escapeAttr(String(s == null ? '' : s));
const pad = (n: number, w = 3): string => String(n).padStart(w, '0');

export const STYLE = `@charset "utf-8";
...
function page(title: string, bodyXhtml: string, lang: string): string {
...
function metaRow(key: string, value: string | undefined): string {
  if (!value) return '';
...
Downloaded ${esc(new Date().toISOString().slice(0, 10))}</p>`,
    book.language,
  );
}

function navPage(book: Book, chapters: Chapter[]): string {
...
function ncx(book: Book, chapters: Chapter[], uid: string): string {
...
function opf(book: Book, chapters: Chapter[], uid: string, cover: CoverRef | null): string {
  const manifest = [
...
export async function build(book: Book): Promise<Blob> {
  const chapters = book.chapters || [];
  const uid = `urn:uuid:${book.uuid || newUuid()}`;
  const lang = book.language || 'en';
  const withLang: Book = { ...book, language: lang };
